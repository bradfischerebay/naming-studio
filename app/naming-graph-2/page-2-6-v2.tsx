...
export default function NamingGraph26Page() {
  ...
  const initGraph = () => {
    ...
    if (preset.sizeByChildren) {
      ...
      nodesData.forEach((node: any) => {
        ...
        if (node.tier === 'master') node.size = 60 + f * 40;
          ...
          node.size = 6 + f * 15 + (node.childCount > 0 ? 6 : 0);
        }
      });
    }
    ...
    const calculateHierarchyDepth = (node: any): number => {
      ...
      while (current?.parent && depth < 20) {
        ...
        if (!parent) break;
        depth++;
        if ((parent as any).tier === 'sub-umbrella' || (parent as any).tier === 'umbrella' || (parent as any).tier === 'master') {
          return depth;
        }
        current = parent;
      }
    ...
    nodesData.forEach((node: any) => {
      if (node.tier !== 'master' && node.tier !== 'umbrella' && node.tier !== 'sub-umbrella') {
    ...
    nodesData.forEach((node: any) => {
      ...
      const tierOffset = depth === 1 ? preset.radial.t1Offset
        : depth === 2 ? preset.radial.t2Offset
        : preset.radial.t3Offset;
      const distVariation = depth === 1 ? 50 : depth === 2 ? 80 : 120;
    ...
    const simulation = d3.forceSimulation(nodesData)
      .force('link', d3.forceLink(linksData).id((d: any) => d.id)
        .distance((d: any) => {
          ...
          const depth = d.source._hierarchyDepth || 0;
          if (depth === 1) return preset.link.t1 * 1.2;
          if (depth === 2) return preset.link.t2 * 1.3;
          if (depth >= 3) return preset.link.t3 * 1.4;
          return preset.link.strength;
        })
        .strength((d: any) => {
          if (d.target._hierarchyDepth) return preset.link.strength * 2;
          return preset.link.strength;
        })
      )
      .force('charge', d3.forceManyBody().strength((d: any) => {
        if (d.tier === 'master' || d.tier === 'umbrella' || d.tier === 'sub-umbrella') return 0;
        const depth = d._hierarchyDepth || 0;
        if (depth === 1) return preset.charge.t1;
        if (depth === 2) return preset.charge.t2;
        if (depth >= 3) return preset.charge.t3;
        return 0;
      }))
      ...
      .force('radial', d3.forceRadial((d: any) => {
        ...
        if (depth === 1) return actualSubUmbrellaRadius + preset.radial.t1Offset * 0.7;
        if (depth === 2) return actualSubUmbrellaRadius + preset.radial.t2Offset * 0.6;
        return actualSubUmbrellaRadius + preset.radial.t3Offset * 0.5;
      }, centerX, centerY).strength((d: any) => {
        if (d.tier === 'master' || d.tier === 'umbrella' || d.tier === 'sub-umbrella') return 0;
        const depth = d._hierarchyDepth || 1;
        if (depth === 1) return 0.03;
        if (depth === 2) return 0.02;
        return 0.01;
      }))
      .force('parentAngle', () => {
    ...
    const getChildrenByDepth = (nodeId: string) => {
      const result = { depth1: [] as any[], depth2: [] as any[], depth3plus: [] as any[] };
      const collect = (parentId: string) => {
        nodesData.forEach((n: any) => {
          if (n.parent === parentId) {
            const depth = n._hierarchyDepth || 1;
            if (depth === 1) result.depth1.push(n);
            else if (depth === 2) result.depth2.push(n);
            else result.depth3plus.push(n);
            collect(n.id);
          }
        });
      };
    ...
    const showTooltipFn = (event: any, d: any) => {
      ...
      const children = getChildrenByDepth(d.id);
      ...
      if (totalChildren > 0) {
        childrenHtml = `<div style="margin-top:10px;padding-top:10px;border-top:1px solid #444">
          ...
          <div style="font-size:11px;max-height:350px;overflow-y:auto">
            ${children.depth1.length > 0 ? `<details style="margin-bottom:6px"><summary style="cursor:pointer;color:#4CAF50;font-weight:bold">Direct Children (${children.depth1.length})</summary><div style="padding-left:12px;margin-top:4px;color:#ccc">${children.depth1.map((c: any) => `<div style="padding:2px 0">• ${c.name} <span style="color:#666;font-size:10px">(${c.tier})</span></div>`).join('')}</div></details>` : ''}
            ${children.depth2.length > 0 ? `<details style="margin-bottom:6px"><summary style="cursor:pointer;color:#2196F3;font-weight:bold">Grandchildren (${children.depth2.length})</summary><div style="padding-left:12px;margin-top:4px;color:#ccc">${children.depth2.map((c: any) => `<div style="padding:2px 0">• ${c.name} <span style="color:#666;font-size:10px">(${c.tier})</span></div>`).join('')}</div></details>` : ''}
            ${children.depth3plus.length > 0 ? `<details style="margin-bottom:6px"><summary style="cursor:pointer;color:#FF9800;font-weight:bold">Deeper Descendants (${children.depth3plus.length})</summary><div style="padding-left:12px;margin-top:4px;color:#ccc">${children.depth3plus.map((c: any) => `<div style="padding:2px 0">• ${c.name} <span style="color:#666;font-size:10px">(${c.tier})</span></div>`).join('')}</div></details>` : ''}
          </div></div>`;
      }
    ...
    node.append('text')
      ...
      .text((d: any) => {
        const maxLen = d.tier === 'master' ? 50
          ...
          : (d._hierarchyDepth === 1 && d.childCount > 0) ? 26
          : 22;
    ...
    (window as any).resetView = () => {
      ...
      node.selectAll('circle').attr('opacity', (d: any) => nodePassesFilters(d) ? 1 : 0.15);
      node.selectAll('text').attr('opacity', (d: any) => nodePassesFilters(d) ? 1 : 0.15);
      link.attr('stroke-opacity', 1).attr('stroke-width', 0.5);
      svg.transition().duration(500).call(
        (zoom as any).transform,
        d3.zoomIdentity.translate(width / 2 - centerX * initialScale, height / 2 - centerY * initialScale).scale(initialScale)
      );
    };
    ...
    (window as any).exportImage = () => {
      ...
      img.onload = () => {
        ...
        a.download = `ebay-naming-graph-v26-${currentMarket}.png`; a.click();
      };
    ...
    <div className="h-screen w-full flex flex-col overflow-hidden" style={{ backgroundColor: theme.bg }}>
      <div className="flex items-center justify-between px-4 py-3 relative z-20" style={{ borderBottom: `1px solid ${theme.border}`, backgroundColor: theme.bg }}>
        ...
        <div className="text-center">
          <h1 className="text-xl font-bold" style={{ color: theme.textPrimary }}>eBay Naming Graph V2.6</h1>
      ...
      <div className="flex-1 flex relative">
        ...
        <div className="absolute top-4 right-4 px-3 py-2 rounded" style={{ backgroundColor: theme.bgCard, border: `1px solid ${theme.border}` }}>
          ...
          <span id="zoom-level" className="text-sm font-mono" style={{ color: theme.textPrimary }}>33%</span>
        </div>
      </div>
